...
class Dataloader(pl.LightningDataModule):
    ...
    def tokenizing(self, dataframe):
        ...
        for idx, item in tqdm(dataframe.iterrows(), desc='tokenizing', total=len(dataframe)):
            ...
            outputs = self.tokenizer(text, add_special_tokens=True, padding='max_length', truncation=True, max_length=128)
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            all_df = pd.concat([train_data,val_data],axis=0).reset_index(drop=True)

            # train_data = train_data.drop(val_data.index,axis = 0)
            
            # 학습데이터 준비
            train_inputs, train_targets = self.preprocessing(all_df)
            ...
            print('all_splits,len',len(all_splits))
...
if __name__ == '__main__':
    ...
    # wandb.login()
    # wandb_logger = WandbLogger(name='k-fold_EDA', project='SST')
