...
def main():
    ...
    dataloader.predict_path = '../data/stacking_electra.csv'
    ...
    ckpt = model.load_from_checkpoint("/opt/ml/aistages/repo/save/ckpt/xlm-roberta-large-re-re-epoch=8-val_pearson=0.9297.ckpt")
    predictions = trainer.predict(model=ckpt, datamodule=dataloader)

    # 예측된 결과를 형식에 맞게 반올림하여 준비합니다.
    predictions = list(round(float(i), 1) for i in torch.cat(predictions))

    # output 형식을 불러와서 예측된 결과로 바꿔주고, output.csv로 출력합니다.
    output = pd.read_csv('../data/stacking_electra.csv')
    output['xlm_pred'] = predictions
    output.to_csv('./save/output/' + "stacking_electra_xlm" + '.csv', index=False)
